...
namespace IBlameYou.Enemies
{
    ...
    public class EnemyController : MonoBehaviour
    {
        ...
        private void OnCollisionStay2D(Collision2D collision)
        {
            ...
            var targetHealth = collision.collider.GetComponentInParent<HealthSystem>();
            if (targetHealth == null || targetHealth == health) return;

            if (lastHitTime.TryGetValue(targetHealth, out float last) && Time.time - last < contactDamageCooldown) return;
